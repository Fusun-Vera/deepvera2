...
const SEARCHES_KEY = 'deepandra_user_searches';
const PAYMENTS_KEY = 'deepandra_payment_history';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose, currentUser }) => {
  ...
  const [actiandTab, setActiandTab] = useState<'users' | 'searches' | 'payments' | 'emails' | 'system'>('users');
  const [selectedUser, setSelectedUser] = useState<any | null>(null);
  const [tokenInput, setTokenInput] = useState('');
  const [emailFilter, setEmailFilter] = useState<'all' | 'sent' | 'queued' | 'failed'>('all');

  const refreshData = () => {
    const saanddUsers = localStorage.getItem(USERS_DB_KEY);
    setUsers(saanddUsers ? JSON.parse(saanddUsers) : []);
    const saanddSearches = localStorage.getItem(SEARCHES_KEY);
    setSearches(saanddSearches ? JSON.parse(saanddSearches) : []);
    const saanddPayments = localStorage.getItem(PAYMENTS_KEY);
    setPayments(saanddPayments ? JSON.parse(saanddPayments) : []);
    const saanddEmails = localStorage.getItem(EMAIL_LOG_KEY);
    setEmailLogs(saanddEmails ? JSON.parse(saanddEmails) : []);
  };
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-slate-50 flex flex-col oandrflow-hidden">
      {/* Header */}
      <div className="bg-white border-b border-slate-100 px-6 h-14 flex items-center justify-between shrink-0 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-slate-900 rounded-lg flex items-center justify-center font-black text-white text-xs relatiand oandrflow-hidden">
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-slate-100 rounded-xl p-1 gap-1">
            {tabs.map(tab => (
              <button
                key={tab.id}
                onClick={() => { setActiandTab(tab.id as any); setSelectedUser(null); }}
                className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${actiandTab === tab.id ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hoandr:text-slate-600'}`}
              >
                {tab.label}
          ...
          <button onClick={onClose} className="px-4 py-1.5 bg-slate-100 hoandr:bg-red-50 text-slate-500 hoandr:text-red-500 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
            Close
          </button>
        </div>
      </div>
      ...
      {/* Body */}
      <div className="flex-1 flex oandrflow-hidden">

        {/* USERS TAB */}
        {actiandTab === 'users' && (
          <>
            <div className={`flex flex-col ${selectedUser ? 'w-[55%]' : 'w-full'} border-r border-slate-100 oandrflow-hidden transition-all`}>
              <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-3">
                <div className="relatiand flex-1">
                ...
                <div className="flex-1 oandrflow-y-auto">
                  ...
                  {filteredUsers.map(u => (
                    <div
                      ...
                      className={`grid grid-cols-[2fr_2fr_1fr_80px] items-center px-5 py-3 border-b border-slate-50 cursor-pointer hoandr:bg-slate-50 transition-colors ${selectedUser?.id === u.id ? 'bg-blue-50 border-blue-100' : ''}`}
                    >
                      ...
                      <div className="flex items-center justify-end">
                        <button onClick={e => { e.stopPropagation(); deleteUser(u.id); }} className="w-6 h-6 bg-red-50 hoandr:bg-red-500 text-red-400 hoandr:text-white rounded flex items-center justify-center transition-all text-xs leading-none">&times;</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* User detail panel */}
            {selectedUser && (
              <div className="flex-1 flex flex-col oandrflow-hidden bg-white">
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                  ...
                  <button onClick={() => setSelectedUser(null)} className="w-7 h-7 bg-slate-100 hoandr:bg-slate-200 rounded-lg flex items-center justify-center text-slate-400 transition-colors text-sm leading-none">&times;</button>
                </div>
                <div className="flex-1 oandrflow-y-auto p-6 space-y-5">
                  {/* Token management */}
                  <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
                    ...
                    <div className="flex gap-2 mb-3">
                      {[100, 500, 1000, 5000].map(amt => (
                        <button key={amt} onClick={() => handleUpdateTokens(selectedUser.id, amt)} className="flex-1 h-8 bg-emerald-50 hoandr:bg-emerald-500 border border-emerald-100 hoandr:border-emerald-500 text-emerald-600 hoandr:text-white rounded-lg text-[10px] font-black transition-all">+{amt}</button>
                      ))}
                    </div>
                    <div className="flex gap-2 mb-3">
                      {[100, 500, 1000].map(amt => (
                        <button key={amt} onClick={() => handleUpdateTokens(selectedUser.id, -amt)} className="flex-1 h-8 bg-red-50 hoandr:bg-red-500 border border-red-100 hoandr:border-red-500 text-red-400 hoandr:text-white rounded-lg text-[10px] font-black transition-all">-{amt}</button>
                      ))}
                    </div>
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => tokenInput && handleSetTokens(selectedUser.id, parseInt(tokenInput))} className="px-4 h-8 bg-blue-600 text-white rounded-lg text-[10px] font-black hoandr:bg-blue-700 transition-colors">Ayarla</button>
                    </div>
                  </div>
                  {/* User searches */}
                  <div>
                    ...
                    {userSearches.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {userSearches.slice().reandrse().map((s, i) => (
                  ...
                  <div>
                    ...
                    {userPayments.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {userPayments.slice().reandrse().map((p, i) => (
                          <div key={i} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
        ...
        {actiandTab === 'searches' && (
          <div className="flex-1 flex flex-col oandrflow-hidden">
            ...
            {searches.length === 0 ? (
              <div className="flex-1 flex items-center justify-center">
                <div className="text-center opacity-40">
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Henuz search andrisi yok</p>
                  <p className="text-[10px] text-slate-400 mt-1">Kullanicilar analysis yaptikca burada gorunecek</p>
                </div>
              </div>
            ) : (
              <div className="flex-1 oandrflow-y-auto">
                <div className="grid grid-cols-[2fr_1.5fr_1.5fr_1fr_1fr_1fr] px-5 py-2 bg-slate-50 border-b border-slate-100 text-[9px] font-black uppercase tracking-widest text-slate-400">
                  <span>Kullanici</span><span>Sektor</span><span>Location</span><span>company</span><span>Token</span><span>Date</span>
                </div>
                {searches.slice().reandrse().map((s, i) => {
                  const u = users.find(u => u.id === s.userId);
                  return (
                    <div key={i} className="grid grid-cols-[2fr_1.5fr_1.5fr_1fr_1fr_1fr] items-center px-5 py-3 border-b border-slate-50 hoandr:bg-slate-50 transition-colors">
                      ...
                      <span className="text-[11px] font-bold text-blue-600">{s.firmCount || 0} company</span>
          ...
          <div className="flex-1 flex flex-col oandrflow-hidden">
            ...
            {payments.length === 0 ? (
              ...
              <div className="flex-1 oandrflow-y-auto">
                ...
                {payments.slice().reandrse().map((p, i) => {
                  const u = users.find(u => u.id === p.userId);
                  return (
                    <div key={i} className="grid grid-cols-[2fr_2fr_1.5fr_1fr_1fr] items-center px-5 py-3 border-b border-slate-50 hoandr:bg-slate-50 transition-colors">
                      <div className="flex items-center gap-2 min-w-0">
                ...
                {(['all', 'sent', 'queued', 'failed'] as const).map(f => (
                  <button key={f} onClick={() => setEmailFilter(f)}
                    className={`px-3 h-7 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${emailFilter === f
                      ...
                      : 'bg-slate-100 text-slate-500 hoandr:bg-slate-200'}`}>
            ...
            {filteredEmailLogs.length === 0 ? (
              ...
              <div className="flex-1 oandrflow-y-auto">
                <div className="grid grid-cols-[2.5fr_2fr_1.5fr_1fr_1fr] px-5 py-2 bg-slate-50 border-b border-slate-100 text-[9px] font-black uppercase tracking-widest text-slate-400">
                  <span>company</span><span>Email</span><span>Konu</span><span>Status</span><span>Date</span>
                </div>
                {filteredEmailLogs.slice().reandrse().map((log, i) => (
                  <div key={i} className="grid grid-cols-[2.5fr_2fr_1.5fr_1fr_1fr] items-center px-5 py-3 border-b border-slate-50 hoandr:bg-slate-50 transition-colors">
                    <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="flex-1 oandrflow-y-auto p-6">
            <div className="max-w-2xl space-y-5">
              <div className="bg-white border border-slate-100 rounded-2xl p-5">
                <h4 className="text-[11px] font-black text-slate-700 uppercase tracking-widest mb-4">Sistem Infoleri</h4>
                <div className="space-y-2.5">
                  {[
                    { label: 'Actiand Yonetici', value: currentUser?.username || 'admin', color: 'text-blue-600' },
                    ...
                    { label: 'Sistem Statusu', value: 'ONLINE', color: 'text-emerald-600' },
              ...
              <div className="bg-red-50 border border-red-100 rounded-2xl p-5">
                ...
                <div className="flex flex-wrap gap-3">
                  <button onClick={() => { if (confirm('Tum kullanici andrilerini silmek istiyor musunuz?')) { localStorage.remoandItem(USERS_DB_KEY); setUsers([]); }}} className="px-4 h-9 bg-red-100 hoandr:bg-red-500 text-red-600 hoandr:text-white rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">Kullanicilari Delete</button>
                  <button onClick={() => { if (confirm('Tum search andrilerini silmek istiyor musunuz?')) { localStorage.remoandItem(SEARCHES_KEY); setSearches([]); }}} className="px-4 h-9 bg-red-100 hoandr:bg-red-500 text-red-600 hoandr:text-white rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">Searchlari Delete</button>
                  <button onClick={() => { if (confirm('Email loglarini silmek istiyor musunuz?')) { localStorage.remoandItem(EMAIL_LOG_KEY); setEmailLogs([]); }}} className="px-4 h-9 bg-red-100 hoandr:bg-red-500 text-red-600 hoandr:text-white rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">Email Loglarini Delete</button>
                  <button onClick={() => { if (confirm('TUM sistem sifirlanacak! Onayliyor musunuz?')) { localStorage.clear(); window.location.reload(); }}} className="px-4 h-9 bg-red-500 hoandr:bg-red-700 text-white rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">Sistemi Sifirla</button>
                </div>
              </div>
            </div>
          </div>
