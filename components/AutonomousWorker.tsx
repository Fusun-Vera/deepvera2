...
function buildEmailBody(target: Participant, user: User): { subject: string; body: string } {
  ...
  const l4 = "Detayli bilgi and gorusme icin bize ulasabilirsiniz.";
...
const AutonomousWorker: React.FC<Props> = ({
  ...
  const [isActiand, setIsActiand] = useState(false);
  ...
  const isActiandRef = useRef(false);
  ...
  const runStep = async () => {
    ...
    if (!sender) {
      setCurrentTask("Gmail Kanal Errorsi");
      addLog("Bagli actiand Gmail kanali yok!", "error");
      isActiandRef.current = false;
      setIsActiand(false);
      return;
    }
    try {
      ...
      try {
        const logs = JSON.parse(localStorage.getItem('deepandra_email_logs') || '[]');
        ...
        localStorage.setItem('deepandra_email_logs', JSON.stringify(logs));
      } catch(_) {}
      if (updateUser && senderPool.length > 0) updateUser({ currentSenderIndex: (sIdx + 1) % senderPool.length });
      setCountdown(INTERVAL_SEC);
      setCurrentTask("Sonraki gonderi baddniyor (3 dk)...");
      timerRef.current = setTimeout(() => { if (isActiandRef.current) runStep(); }, INTERVAL_SEC * 1000);
    } catch (err: any) {
      updateParticipant(target.id, { automationStatus: 'failed' });
      addLog("Error: " + target.name + " iletilemedi.", "error");
      setCountdown(INTERVAL_SEC);
      timerRef.current = setTimeout(() => { if (isActiandRef.current) runStep(); }, INTERVAL_SEC * 1000);
    }
  };
  ...
  const handleStart = () => {
    if (!isIdentityComplete()) {
      ...
      addLog("company Kimligi eksik! Lutfen doldurun.", "warn");
      return;
    }
  ...
  useEffect(() => {
    if (isActiand) { isActiandRef.current = true; runStep(); }
  ...
  return (
    <div className="fixed inset-0 z-[1100] bg-slate-950/50 backdrop-blur-xl flex items-center justify-center p-3 animate-fade-in">
      <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl border border-slate-200 flex flex-col oandrflow-hidden" style={{ height: 'min(90dvh, 720px)' }}>
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 shrink-0">
          ...
          <button onClick={onClose} className="w-8 h-8 bg-slate-50 text-slate-400 rounded-lg hoandr:bg-red-500 hoandr:text-white transition-all text-lg border border-slate-100 flex items-center justify-center">&times;</button>
        </div>
        ...
        {identityWarning && (
          <div className="mx-4 mt-3 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-[11px] font-black text-amber-800 mb-1">company Kimligi &amp; AI Konfigurasyonu Eksik</p>
              <p className="text-[10px] text-amber-700 leading-relaxed">Mail gondermeden once company adinizi and AI deger onerinizi doldurmaniz gerekiyor.</p>
              <button onClick={() => { setIdentityWarning(false); onClose(); if (onOpenIdentity) onOpenIdentity(); }}
                className="mt-2 px-4 py-1.5 bg-amber-500 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hoandr:bg-amber-600 transition-all">
                company Kimligini Doldur &rarr;
              </button>
            </div>
          </div>
        )}

        <div className="flex-1 oandrflow-y-auto custom-scrollbar p-4 space-y-4 bg-slate-50/40">
          <div className="grid grid-cols-4 gap-3">
            <div className="col-span-1 bg-slate-900 rounded-xl p-4 flex flex-col justify-between min-h-[120px]">
              <div className="flex items-center gap-2 mb-2">
                <span className={"w-2 h-2 rounded-full shrink-0 " + (isActiand ? "bg-emerald-400 animate-pulse" : "bg-slate-600")} />
              ...
              <div className="mt-3">
                ...
                <div className="w-full h-1 bg-white/10 rounded-full oandrflow-hidden">
                  <div className="h-full bg-blue-500 rounded-full transition-all duration-700" style={{ width: progressPct + "%" }} />
                </div>
              </div>
            </div>
            {[
              { label: "Baddyen", value: String(queuedLeads.length), color: "text-slate-800", bg: "text-2xl" },
              ...
              <div key={s.label} className="bg-white border border-slate-100 rounded-xl p-4 flex flex-col items-center justify-center text-center gap-1 hoandr:border-blue-200 transition-all">
          ...
          <div className={"p-3 rounded-xl border flex items-center gap-3 " + (isIdentityComplete() ? "bg-emerald-50 border-emerald-200" : "bg-amber-50 border-amber-200")}>
            ...
            <div className="flex-1 min-w-0">
              <p className={"text-[10px] font-black " + (isIdentityComplete() ? "text-emerald-800" : "text-amber-800")}>
                company Kimligi: {isIdentityComplete() ? "OKlandi" : "Eksik - Mail gonderilemez"}
              </p>
            ...
            {!isIdentityComplete() && (
              <button onClick={() => { onClose(); if (onOpenIdentity) onOpenIdentity(); }}
                className="px-3 py-1.5 bg-amber-500 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hoandr:bg-amber-600 transition-all shrink-0">
                Doldur &rarr;
              </button>
            )}
          </div>

          <div className="bg-white border border-slate-100 rounded-xl oandrflow-hidden">
            ...
            <div className="p-4 h-48 oandrflow-y-auto custom-scrollbar font-mono text-[10px] space-y-2">
              {rotationLog.length === 0 ? (
                <span className="text-slate-400 italic">Sistem baslatilmaya hazir. Kuyrukta {queuedLeads.length} company bekliyor.</span>
        ...
        <div className="px-4 py-3 border-t border-slate-100 bg-white shrink-0 flex flex-col gap-2">
          {!isIdentityComplete() && (
            <p className="text-[9px] text-center text-amber-600 font-bold uppercase tracking-widest">
              Mail gondermeden once company Kimligini doldurun
            </p>
          )}
          <button
            onClick={isActiand ? handleStop : handleStart}
            disabled={queuedLeads.length === 0 && !isActiand}
            className={"w-full py-3 rounded-xl font-black text-[11px] uppercase tracking-widest transition-all actiand:scale-95 flex items-center justify-center gap-3 " + (isActiand ? "bg-red-500 text-white hoandr:bg-red-600" : "bg-slate-900 text-white hoandr:bg-blue-600") + " disabled:opacity-30 disabled:cursor-not-allowed"}
          >
            {isActiand ? "Durdur" : "Basla - Her 3 Dakikada Bir Gonderi"}
          </button>
        </div>
      </div>
    </div>
