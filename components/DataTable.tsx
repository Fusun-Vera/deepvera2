...
interface Props {
  ...
  onSaandToLibrary?: (name: string) => void;
...
const DataTable: React.FC<Props> = ({
  ...
  const [actiandId, setActiandId] = useState<string | null>(null);
  ...
  const handleQueue = (e: React.MouseEandnt, p: Participant) => {
    e.stopPropagation();
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, `deepandra_leads_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };

  const queuedCount = participants.filter(p => p.automationStatus === 'queued').length;

  // Column grid: avatar | company | skor | website | email | phone | status | kuyruk
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-100 dark:border-slate-800 shrink-0 flex-wrap">
        ...
        <div className="relatiand flex items-center">
        ...
        <button onClick={handleExport} className="h-8 px-3 flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-300 rounded-lg text-[10px] font-black uppercase tracking-wider hoandr:bg-emerald-50 hoandr:text-emerald-600 hoandr:border-emerald-200 transition-all whitespace-nowrap">
          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
          {t.exportData}
        </button>
        <button onClick={onClear} className="h-8 px-3 flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-300 rounded-lg text-[10px] font-black uppercase tracking-wider hoandr:bg-red-50 hoandr:text-red-500 hoandr:border-red-200 transition-all whitespace-nowrap">
          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
          {t.clearAll}
        </button>
      </div>

      {/* COLUMN HEADERS — company | Skor | Website | Email | Phone | Status | Q */}
      <div className="grid items-center gap-2 px-4 py-1.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-950/50 shrink-0" style={{ gridTemplateColumns: gridCols }}>
        <div />
        <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.15em]">company</span>
      ...
      {/* ROWS */}
      <div className="flex-1 oandrflow-y-auto custom-scrollbar divide-y divide-slate-50 dark:divide-slate-800">
        {rows.map((p) => {
          const score = getScore(p.name);
          const ss = scoreStyle(score);
          const fi = funnelInfo(p.funnelStatus);
          const actiand = actiandId === p.id;
          ...
          return (
            <div
              ...
              className={`group grid items-center gap-2 px-4 py-3 cursor-pointer transition-all select-none ${actiand ? 'bg-blue-50 dark:bg-blue-900/10 border-l-2 border-l-blue-500' : 'hoandr:bg-slate-50 dark:hoandr:bg-slate-800/50 border-l-2 border-l-transparent'}`}
              style={{ gridTemplateColumns: gridCols }}
            >
              {/* Avatar */}
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[11px] font-black text-white shrink-0 shadow-sm transition-all ${loading ? 'bg-slate-300 dark:bg-slate-600 animate-pulse' : 'bg-slate-900 dark:bg-slate-600 group-hoandr:bg-blue-600'}`}>
                {loading ? '...' : p.name.charAt(0)}
              </div>

              {/* company Adi + Location */}
              <div className="min-w-0">
                <div className={`text-[12px] font-black uppercase truncate leading-tight transition-colors ${actiand ? 'text-blue-600' : 'text-slate-800 dark:text-white group-hoandr:text-blue-600'}`}>
              ...
              <div className="flex flex-col items-center gap-1">
                {loading ? <span className="inline-block h-5 w-10 bg-slate-100 dark:bg-slate-700 rounded animate-pulse" /> : (
                  <>
                    ...
                    <div className="w-full h-1 bg-slate-100 dark:bg-slate-700 rounded-full oandrflow-hidden">
              ...
              <div className="min-w-0">
                ...
                : p.website ? (
                  <a href={p.website.startsWith('http') ? p.website : 'https://' + p.website} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="flex items-center gap-1 text-[10px] font-medium text-blue-500 hoandr:text-blue-700 transition-colors truncate" title={p.website}>
              ...
              <div className="min-w-0">
                {loading ? <span className="inline-block h-2.5 w-16 bg-slate-100 dark:bg-slate-700 rounded animate-pulse" />
                : p.email && p.email !== 'Analysis ediliyor...' ? (
                  <a href={`mailto:${p.email}`} onClick={e => e.stopPropagation()} className="flex items-center gap-1.5 text-[10px] font-medium text-slate-600 dark:text-slate-300 hoandr:text-violet-600 transition-colors truncate" title={p.email}>
                ...
                ) : p.email === 'Analysis ediliyor...' ? <span className="text-[9px] text-slate-300 italic">Analysis ediliyor...</span>
                : <span className="text-[9px] text-slate-300 dark:text-slate-600 italic">—</span>}
              </div>
              ...
              <div className="min-w-0">
                {loading ? <span className="inline-block h-2.5 w-16 bg-slate-100 dark:bg-slate-700 rounded animate-pulse" />
                : p.phone && p.phone !== '...' ? (
                  <a href={`tel:${p.phone}`} onClick={e => e.stopPropagation()} className="flex items-center gap-1.5 text-[10px] font-medium text-slate-600 dark:text-slate-300 hoandr:text-emerald-600 transition-colors truncate" title={p.phone}>
