...
const COMPANY_INFO = {
  unvan: 'Aİ Müzik Yazılım Pazarlama İthalat İhracat Ticaret Limited Şirketi',
...
const LandingPage: React.FC<LandingPageProps> = ({ onGetStarted, onToggleAssistant, language, onLanguageChange }) => {
  ...
  const faqs = isTr ? [
    ...
    { q: 'Kac firmaya email gonderilebilir?', a: '20 farkli Gmail hesabi ekleyebilir, her 3 dakikada bir farkli hesaptan peske email gonderilebilir. Hangi hesaptan kime mail gittigi ve alinan cevaolar panelden izlenebilir.' },
    { q: 'Uluslararası kullanım mumkun mu?', a: 'Evet. Turkiye dahil Almanya, ABD, Ingiltere, Fransa, Kanada, Avustralya ve Afrika ulkeleri desteklenmektedir.' },
  ] : [
    ...
    { q: 'Which sectors can I use it for?', a: 'Restaurant, insurance, logistics, health, HORECA, manufacturing, consulting, software, automotive, fashion, beauty and many more. You can also enter custom sector terms.' },
    ...
    { q: 'How many emails can be sent?', a: 'You can add 20 different Gmail accounts, sending emails from different accounts every 3 minutes in sequence. Track which account sent to whom and total replies in the dashboard.' },
    { q: 'Is international use possible?', a: 'Yes. Turkey, Germany, USA, UK, France, Canada, Australia and African countries are supported.' },
  ];

  const sectors = [
    { icon: '🛡️', title: isTr ? 'Sigorta & Finans' : 'Insurance & Finance', metric: '+35%', desc: isTr ? 'Yeni police & portfoy buyumesi' : 'New policy & portfolio growth' },
    ...
    { icon: '🏥', title: isTr ? 'Saglik & Turizm' : 'Health & Tourism', metric: '+30%', desc: isTr ? 'Yeni hasta & turist lead' : 'New patient & tourist leads' },
    ...
    { icon: '💼', title: isTr ? 'Danismanlik & B2B' : 'Consulting & B2B', metric: '+40%', desc: isTr ? 'Donusum orani artisi' : 'Conversion rate increase' }
  ];

  const steps = [
    { num: '01', title: isTr ? 'Hedefini Tanimla' : 'Define Your Target', desc: isTr ? '250 Milyon firmanin icinden, ulke, sehir ve sektor secin. Kac firma analiz edilsin? 10 ila 250 arasinda tek tikla secin.' : 'From 250 million companies, select country, city and sector. How many to analyze? 10 to 250 in one click.', color: 'blue' },
    { num: '02', title: isTr ? 'AI Analizi Baslat' : 'Launch AI Analysis', desc: isTr ? 'Derin Analiz butonuna basin. AI motoru 250 milyon firma veritabanini tarar, hedef firmalari bulur ve her biri icin derin profil cikarir.' : 'Press Deep Analysis. AI engine scans 250 million companies, finds targets and extracts deep profiles for each.', color: 'violet' },
    { num: '03', title: isTr ? 'Sonuclari Incele' : 'Review Results', desc: isTr ? 'Firma adi, skor, web sitesi, email ve telefon ile listelenen leads listesini inceleyin. Her karti tikl ayarak detaylari acin.' : 'Browse leads with company name, score, website, email and phone. Click each card to view details.', color: 'emerald' },
    { num: '04', title: isTr ? 'Otonom Satisa Gec' : 'Switch to Autonomous Sales', desc: isTr ? '20 Gmail hesabiyla otonom email gonderimi baslatIn. Her 3 dakikada bir peske farkli hesaptan mail gitsin, yanitlar takip edilsin.' : 'Launch autonomous email with 20 Gmail accounts. Emails sent every 3 min from different accounts, replies tracked automatically.', color: 'orange' },
  ];

  const features = [
    { icon: '🔍', title: isTr ? 'Akilli Firma Tarama' : 'Smart Company Scan', desc: isTr ? '250 Milyon firmalik veritabaninda ulke, sehir ve sektor bazli hedefli tarama. Saniyeler icinde binlerce firma.' : '250 million company database, targeted scanning by country, city and sector. Thousands of companies in seconds.', tag: 'Neural Crawling' },
    { icon: '🧠', title: isTr ? 'Derin Firma Analizi' : 'Deep Company Analysis', desc: isTr ? 'Dijital ayak izi, rakip analizi, pazar konumu ve buyume potansiyeli cikarilir. Gercek zamanli web zekasi.' : 'Digital footprint, competitor analysis, market position and growth potential extracted. Real-time web intelligence.', tag: 'Cognitive Engine' },
    { icon: '📧', title: isTr ? 'Otonom Email Ajani' : 'Autonomous Email Agent', desc: isTr ? '20 Gmail hesabi ekle, her 3 dakikada bir peske farkli hesaptan mail git, yanitlar otomatik takip edilsin.' : 'Add 20 Gmail accounts, send from different accounts every 3 minutes in sequence, replies automatically tracked.', tag: 'Outreach Proxy' },
    { icon: '📊', title: isTr ? 'Canli Istihbarat Paneli' : 'Live Intelligence Dashboard', desc: isTr ? 'Firma - Skor - Web Sitesi - Email - Telefon sirasinda tum leads tek ekranda. Skor bazli onceliklendirme.' : 'Company - Score - Website - Email - Phone order, all leads on one screen. Score-based prioritization.', tag: 'Command Center' },
  ];
    ...
    <div className="bg-white text-slate-900 min-h-screen font-sans overflow-x-hidden">
      ...
      <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-xl border-b border-slate-100 shadow-sm h-16' : 'h-20 bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-6 h-full flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-slate-900 rounded-xl flex items-center justify-center font-black text-white text-sm relative overflow-hidden shadow">
          ...
          <div className="hidden md:flex items-center gap-6">
            <a href="#how" className="text-[11px] font-bold text-slate-500 hover:text-blue-600 transition-colors">{isTr ? 'Nasil Calisir' : 'How It Works'}</a>
            <a href="#features" className="text-[11px] font-bold text-slate-500 hover:text-blue-600 transition-colors">{isTr ? 'Ozellikler' : 'Features'}</a>
            <a href="#sectors" className="text-[11px] font-bold text-slate-500 hover:text-blue-600 transition-colors">{isTr ? 'Sektorler' : 'Sectors'}</a>
            <a href="#faq" className="text-[11px] font-bold text-slate-500 hover:text-blue-600 transition-colors">FAQ</a>
            <a href="#contact" className="text-[11px] font-bold text-slate-500 hover:text-blue-600 transition-colors">{isTr ? 'Iletisim' : 'Contact'}</a>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={onGetStarted} className="px-5 py-2 bg-slate-900 text-white rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-blue-600 transition-colors shadow-md">{isTr ? 'Sisteme Giris' : 'Sign In'}</button>
          </div>
        </div>
      </nav>
        ...
        <div className="max-w-7xl mx-auto px-6 w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center py-16">
          <div className="space-y-8">
            <div className="inline-flex items-center gap-2.5 px-4 py-1.5 bg-blue-50 border border-blue-100 rounded-full">
              ...
              <span className="text-[10px] font-black text-blue-600 uppercase tracking-[0.25em]">{isTr ? 'Otonom Satis Ajanlari Aktif' : 'Autonomous Sales Agents Active'}</span>
            </div>
            ...
            <p className="text-lg text-slate-500 font-medium leading-relaxed max-w-lg">
              {isTr ? '250 Milyon firmanin icinden hedef firmalari seciyor, analiz ediyor ve sizin yerinize ilk temasi kuruyor.' : 'From 250 million companies, selects your targets, analyzes them and makes first contact on your behalf.'}
            </p>
            <div className="flex flex-wrap gap-3">
              <button onClick={onGetStarted} className="px-8 py-4 bg-blue-600 text-white rounded-2xl font-black text-[12px] uppercase tracking-wider hover:bg-slate-900 transition-all shadow-xl shadow-blue-200 active:scale-95">{isTr ? 'Ucretsiz Basla' : 'Start Free'}</button>
              <button onClick={onToggleAssistant} className="px-8 py-4 border-2 border-slate-100 rounded-2xl text-[12px] font-black uppercase tracking-wider hover:border-blue-300 hover:text-blue-600 transition-all">{isTr ? "AI'ya Sor" : 'Ask AI'}</button>
            </div>
            <div className="flex flex-wrap gap-6 pt-2">
              {[{label:isTr?'250M Firma':'250M Companies',icon:'🌍'},{label:isTr?'Aninda kullan':'Instant use',icon:'⚡'},{label:isTr?'AI destekli':'AI powered',icon:'🧠'}].map((item,idx)=>(
          ...
          <div className="hidden lg:block relative">
            <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl shadow-slate-200/80 p-7 space-y-3">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">{isTr ? 'Nasil Calisir' : 'How It Works'}</span>
                </div>
                <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full">{isTr ? 'Canli' : 'Live'}</span>
              </div>
              {/* Step 1 */}
              <div className="flex items-start gap-4 p-4 bg-blue-600 rounded-2xl shadow-lg shadow-blue-200">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[12px] font-black text-white mb-1">{isTr ? 'Hedef Tanimla — 250M Firma' : 'Define Target — 250M Companies'}</div>
                  <div className="flex gap-1.5 flex-wrap">
                    ...
                    <span className="text-[9px] bg-white/20 text-white px-2 py-0.5 rounded-lg font-bold">50 {isTr ? 'Firma' : 'Companies'}</span>
                  </div>
                </div>
              ...
              <div className="flex items-start gap-4 p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[12px] font-black text-slate-900 mb-1">{isTr ? 'AI Tarama & Derin Analiz' : 'AI Scanning & Deep Analysis'}</div>
                  <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden mb-1.5"><div className="h-full w-11/12 bg-gradient-to-r from-blue-600 to-blue-400 rounded-full" /></div>
                  <div className="flex gap-3"><span className="text-[9px] font-bold text-slate-400">{isTr ? 'Web Tarandi' : 'Web Scanned'}</span><span className="text-slate-300">|</span><span className="text-[9px] font-bold text-blue-600">50 {isTr ? 'Firma Bulundu' : 'Companies Found'}</span></div>
                </div>
              </div>
              <div className="flex justify-center"><svg width="16" height="20" viewBox="0 0 16 20" fill="none"><path d="M8 0 L8 14 M8 14 L3 9 M8 14 L13 9" stroke="#CBD5E1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></div>
              ...
              <div className="flex items-start gap-4 p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[12px] font-black text-slate-900 mb-2">{isTr ? 'Firma / Skor / Web / Email / Tel' : 'Company / Score / Web / Email / Phone'}</div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <div className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-xl px-2.5 py-1.5 shadow-sm">
                      ...
                      <span className="text-[9px] font-black text-slate-700">{isTr ? '3 Yanit Alindi' : '3 Replies Received'}</span>
                    </div>
            ...
            <div className="absolute -top-4 -right-4 bg-slate-900 text-white rounded-2xl px-4 py-2.5 shadow-xl z-20">
              ...
              <div className="text-[7px] font-black uppercase tracking-wider mt-0.5 text-white">{isTr ? 'FIRMA' : 'COMPANIES'}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <div ref={statsRef} className="border-y border-slate-100 bg-slate-950 py-10">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-3 gap-8">
          {[
            { value: count1 >= 250000000 ? '250M' : count1.toLocaleString('tr-TR'), suffix: '+', label: isTr ? 'Firma Veritabani' : 'Company Database' },
            { value: count2, suffix: '%', label: isTr ? 'Veri Dogruluk Orani' : 'Data Accuracy Rate' },
            { value: count3, suffix: '+', label: isTr ? 'Aktif Sektor' : 'Active Sectors' }
          ].map((stat, idx) => (
      ...
      <section id="how" className="py-28 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-[10px] font-black text-blue-600 uppercase tracking-[0.4em] block mb-3">{isTr ? 'Nasil Calisir' : 'How It Works'}</span>
            ...
            <p className="text-slate-500 mt-4 max-w-xl mx-auto text-sm font-medium">{isTr ? '250 Milyon firma veritabaninda hedefleri bul, analiz et, otomatik email gonder.' : 'Find targets in 250 million company database, analyze and send automated emails.'}</p>
          </div>
      ...
      <section id="features" className="py-28 bg-slate-950">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl lg:text-5xl font-black tracking-tighter text-white">{isTr ? 'Her Sey Tek Platformda' : 'Everything in One Platform'}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {features.map((f, idx) => (
              <div key={idx} className="group p-8 bg-white/5 border border-white/10 rounded-2xl hover:border-blue-500/40 transition-all duration-300">
                <div className="flex items-start gap-5">
                  <div className="w-12 h-12 bg-white/10 rounded-xl flex items-center justify-center text-2xl shrink-0 group-hover:scale-110 transition-transform">{f.icon}</div>
          ...
          <div className="mt-10 text-center">
            <button onClick={onGetStarted} className="px-10 py-4 bg-blue-600 text-white rounded-2xl font-black text-[12px] uppercase tracking-wider hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/20 active:scale-95">{isTr ? 'Hemen Deneyin' : 'Try It Now'}</button>
          </div>
        </div>
      </section>
      ...
      <section id="sectors" className="py-28 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl lg:text-5xl font-black tracking-tighter text-slate-900">{isTr ? 'Her Sektorde Sonuc' : 'Results in Every Sector'}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {sectors.map((s, idx) => (
              <div key={idx} className="group flex items-center gap-5 p-5 bg-slate-50 rounded-2xl border border-slate-100 hover:border-blue-200 hover:bg-white hover:shadow-lg transition-all duration-300 cursor-default">
                <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-2xl shadow-sm border border-slate-100 shrink-0 group-hover:scale-110 transition-transform">{s.icon}</div>
                <div className="flex-1 min-w-0">
      ...
      <section className="py-24 bg-blue-600 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 text-center relative z-10">
          <h2 className="text-4xl lg:text-6xl font-black text-white tracking-tighter leading-tight mb-6">
            {isTr ? 'Rakipleriniz Beklerken Siz Harekete Gecin.' : 'While Competitors Wait, You Take Action.'}
          </h2>
          <p className="text-blue-100 text-lg font-medium max-w-xl mx-auto mb-10">
            {isTr ? '250 Milyon firma veritabani hazir. Sisteme giris yapin ve ilk analizinizi baslatIn.' : '250 million company database ready. Sign in and start your first analysis.'}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button onClick={onGetStarted} className="px-10 py-4 bg-white text-blue-600 rounded-2xl font-black text-[13px] uppercase tracking-wider hover:bg-slate-900 hover:text-white transition-all shadow-2xl active:scale-95">{isTr ? 'Sisteme Giris Yap' : 'Sign In to System'}</button>
            <button onClick={onToggleAssistant} className="px-10 py-4 border-2 border-white/30 text-white rounded-2xl font-black text-[13px] uppercase tracking-wider hover:border-white transition-all">{isTr ? 'AI Asistanla Konus' : 'Talk to AI Assistant'}</button>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 pb-12 border-b border-white/5">
            <div className="lg:col-span-1">
              ...
              <p className="text-slate-400 text-sm font-medium leading-relaxed max-w-xs">{isTr ? 'B2B satis sureclerinizi yapay zeka ile otomatize edin. 250M firma veritabani.' : 'Automate your B2B sales with AI. 250M company database.'}</p>
            </div>
            {/* Company Info */}
            <div className="lg:col-span-2">
              <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 mb-4">{isTr ? 'Sirket Bilgileri' : 'Company Information'}</h4>
              <div className="space-y-2">
                {[
                  { label: isTr ? 'Unvan' : 'Company', value: COMPANY_INFO.unvan },
                  { label: isTr ? 'Adres' : 'Address', value: COMPANY_INFO.adres },
                  { label: isTr ? 'Vergi Dairesi' : 'Tax Office', value: `${COMPANY_INFO.vergiDairesi} - ${COMPANY_INFO.vergiNo}` },
                ].map((item, i) => (
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">{isTr ? 'Telefon' : 'Phone'}</div>
                  <a href={`tel:${COMPANY_INFO.telefon}`} className="text-lg font-black hover:text-blue-400 transition-colors">{COMPANY_INFO.telefon}</a>
                </div>
                <div>
                  <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">{isTr ? 'E-posta' : 'Email'}</div>
                  <a href={`mailto:${COMPANY_INFO.email}`} className="text-base font-black hover:text-blue-400 transition-colors">{COMPANY_INFO.email}</a>
                </div>
              </div>
            </div>
          </div>
          <div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">&#169; 2025 DeepVera &#8212; {COMPANY_INFO.unvan}</span>
            <div className="flex gap-6">
              <button onClick={onGetStarted} className="text-[10px] font-black text-slate-600 uppercase tracking-widest hover:text-blue-400 transition-colors">{isTr ? 'Giris Yap' : 'Sign In'} &rarr;</button>
              <button onClick={onToggleAssistant} className="text-[10px] font-black text-slate-600 uppercase tracking-widest hover:text-blue-400 transition-colors">AI Assistant &rarr;</button>
            </div>
          </div>
        </div>
      </footer>
      ...
      {/* AI Floating Button */}
      <button onClick={onToggleAssistant} className="fixed bottom-8 right-8 z-[110] w-14 h-14 bg-emerald-500 text-white rounded-full flex flex-col items-center justify-center shadow-[0_15px_40px_rgba(16,185,129,0.35)] hover:scale-110 hover:bg-slate-900 transition-all active:scale-95 group">
        <div className="absolute -inset-1.5 bg-emerald-400 rounded-full blur opacity-20 group-hover:opacity-40 animate-pulse" />
        <div className="text-xl relative z-10">&#129302;</div>
        <span className="text-[6px] font-black uppercase tracking-tighter mt-0.5 relative z-10">DV_AI</span>
      </button>
