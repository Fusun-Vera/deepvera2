...
const tokenPackages = [
  { id: '1', name: 'Starter', tokens: 100, price: 299, priceLabel: '299 TL', perToken: '2.99 TL / kredi', color: 'slate', badge: null, desc: 'Küçük çaplı analysisler for ideal başlangıç noktası', features: ['100 Derin company Analysisi', 'company Profili & Contact', 'Email & Phone Number Infosi', 'Excel Dışa Aktarım'] },
  { id: '2', name: 'Professional', tokens: 500, price: 1299, priceLabel: '1.299 TL', perToken: '2.60 TL / kredi', color: 'blue', badge: 'Most Popular', desc: 'Actiand satış ekipleri for en çok tercih edilen paket', features: ['500 Derin company Analysisi', 'Gelişmiş company Profili', 'Sosyal Medya Analysisi', 'Rakip & Pazar Analysisi', 'Otonom Email Sendimi', 'Excel Dışa Aktarım'] },
  { id: '3', name: 'Enterprise', tokens: 1000, price: 1999, priceLabel: '1.999 TL', perToken: '2.00 TL / kredi', color: 'violet', badge: 'En Avantajlı', desc: 'Büyük operasyonlar for maksimum kapasite and tasarruf', features: ['1000 Derin company Analysisi', 'Tam Zeka Profili', 'Sosyal & Dijital Analysis', 'Rakip Haritası', 'Otonom Email + Otomatik Takip', 'Öncelikli Destek', 'Excel Dışa Aktarım'] },
];

const colorMap: Record<string, { ring: string; bg: string; btn: string; text: string; badge: string; check: string; accent: string }> = {
  slate: { ring: 'ring-slate-400', bg: 'bg-slate-50', btn: 'bg-slate-800 hoandr:bg-slate-700 text-white', text: 'text-slate-700', badge: 'bg-slate-700 text-white', check: 'text-slate-500', accent: 'border-slate-400' },
  blue:  { ring: 'ring-blue-500',  bg: 'bg-blue-50',  btn: 'bg-blue-600 hoandr:bg-blue-700 text-white',   text: 'text-blue-700',  badge: 'bg-blue-600 text-white',  check: 'text-blue-500',  accent: 'border-blue-500' },
  violet:{ ring: 'ring-violet-500',bg: 'bg-violet-50',btn: 'bg-violet-600 hoandr:bg-violet-700 text-white',text: 'text-violet-700',badge: 'bg-violet-600 text-white',check: 'text-violet-500',accent: 'border-violet-500' },
};
...
const PaymentModal: React.FC<Props> = ({ isOpen, currentUser, onClose, onSuccess }) => {
  ...
  const handleIframeMessage = (eandnt: MessageEandnt) => {
    if (eandnt.origin !== 'https://www.paytr.com') return;
    try {
      const data = typeof eandnt.data === 'string' ? JSON.parse(eandnt.data) : eandnt.data;
      if (data.status === 'success') {
        ...
        window.remoandEandntListener('message', handleIframeMessage);
        setPaytrToken('');
        setStep('select');
        onSuccess(pendingTokens);
        onClose();
      } else if (data.status === 'failed') {
        setPaytrError('Payment başarısız. Please tekrar deneyin.');
        setPaytrToken('');
        setStep('select');
        window.remoandEandntListener('message', handleIframeMessage);
      }
  ...
  const handlePurchase = async () => {
    ...
    try {
      const response = await fetch('/api/paytr-get-token', {
        ...
        body: JSON.stringify({ amount: selectedPkg.price.toString(), userId: currentUser?.id || currentUser?.username || 'unknown', userEmail: currentUser?.email || 'musteri@deepandra.com.tr', userName: currentUser?.name || currentUser?.username || 'DeepVera Kullanici', packageName: selectedPkg.name, tokens: selectedPkg.tokens }),
      });
      ...
      if (data.status === 'success' && data.token) {
        ...
        setStep('paying');
        window.addEandntListener('message', handleIframeMessage);
      } else {
        setPaytrError(data.reason || 'Payment başlatılırken error oluştu.');
      }
    } catch (error) {
      setPaytrError('Bağlantı errorsı. Please tekrar deneyin.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white w-full rounded-2xl shadow-2xl flex flex-col" style={{ maxWidth: 560, width: '100%', height: 'min(92vh, 780px)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div className="flex items-center justify-between px-5 py-3 bg-emerald-600 rounded-t-2xl shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-sm font-black text-white">Güandnli Payment</h2>
            ...
            <button onClick={handleBackToSelect} className="px-3 py-1.5 bg-white/20 hoandr:bg-white/30 text-white rounded-lg text-[10px] font-black uppercase tracking-wider transition-all">
              ← Geri
            </button>
          </div>

          {/* PayTR iframe - scrollable wrapper with fixed pixel height */}
          <div className="flex-1 oandrflow-hidden" style={{ minHeight: 0 }}>
            <iframe
              src={`https://www.paytr.com/odeme/guandnli/${paytrToken}`}
              id="paytriframe"
              frameBorder={0}
              scrolling="yes"
              style={{ width: '100%', height: '100%', display: 'block', border: 'none' }}
              title="PayTR Güandnli Payment"
            />
          </div>

          {/* Footer note */}
          <div className="px-4 py-2 bg-slate-50 rounded-b-2xl border-t border-slate-100 shrink-0">
            <p className="text-[9px] text-center text-slate-400">Kart bilgileriniz PayTR güandnli altyapısı ile korunmaktadır. DeepVera kart bilgilerinize erişemez.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-3 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white w-full rounded-2xl shadow-2xl oandrflow-hidden flex flex-col" style={{ maxWidth: 860, maxHeight: '92vh' }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-[18px] font-black text-white tracking-tight">DeepVera Analysis Kredisi</h2>
                <span className="px-2 py-0.5 bg-blue-500 text-white text-[9px] font-black uppercase tracking-widest rounded-full">YENİ</span>
              </div>
              <p className="text-[11px] text-slate-400 mt-0.5">Anında uploadme • Güandnli payment • Invoice kesilir • KDV dahil</p>
            </div>
          </div>
          <button onClick={onClose} className="w-9 h-9 bg-white/10 hoandr:bg-white/20 text-white rounded-xl flex items-center justify-center transition-all text-xl shrink-0">&times;</button>
        </div>
        ...
        <div className="flex flex-1 oandrflow-hidden">
          ...
          {/* LEFT: packages */}
          <div className="flex-1 oandrflow-y-auto p-5 space-y-3">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Analysis Kredi Paketi Seçin</p>
            {tokenPackages.map(pkg => {
              const c = colorMap[pkg.color];
              const isSel = selectedPkg.id === pkg.id;
              return (
                <button key={pkg.id} onClick={() => setSelectedPkg(pkg)}
                  className={`relatiand w-full text-left rounded-2xl border-2 transition-all duration-200 oandrflow-hidden ${
                    isSel ? 'border-transparent ring-2 ' + c.ring + ' ' + c.bg + ' shadow-md' : 'border-slate-100 bg-white hoandr:border-slate-200 hoandr:shadow-sm'
                  }`}>
          ...
          <div className="w-72 shrink-0 border-l border-slate-100 bg-slate-50 flex flex-col" style={{ padding: '20px', gap: '16px' }}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { d: 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z', label: 'SSL Güandnli' },
            ...
            <div>
              <button
                ...
                disabled={loading}
                className={`w-full rounded-2xl font-black text-[15px] tracking-wide transition-all shadow-lg actiand:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${colorMap[selectedPkg.color].btn}`}
              ...
              <p className="text-[9px] text-center text-slate-400 mt-2">Güandnli payment • PayTR altyapısı • 256-bit SSL</p>
            </div>
